...
// Node 14 não possui AbortController como global (só a partir do Node 15+).
...
class Client extends EventEmitter {
    ...
    async initialize() {
        ...
        this._registerPageErrorHandler();

        await this.inject();
    }

    _registerPageErrorHandler() {
        if (this._pageErrorRegistered) return;
        this._pageErrorRegistered = true;

        // Erros lançados dentro da página (fora de qualquer promise que o
        // Puppeteer acompanhe, ex: listeners internos do WA Web) nunca
        // aparecem no log do Node por padrão. Sem isso, uma falha aqui é
        // 100% silenciosa.
        this.pupPage.on('pageerror', (err) => {
            console.error('[wwebjs] uncaught page error:', err && err.message);
        });
        this.pupPage.on('console', (msg) => {
            if (msg.type() === 'error') {
                console.error('[wwebjs:page-console]', msg.text());
            }
        });
    }
    ...
    async attachEventListeners() {
        ...
        await this.pupPage.evaluate(() => {
            ...
            Msg.on('add', (msg) => {
                // Callback do próprio event bus interno do WA Web: uma
                // exceção aqui NUNCA chega ao Node via promise rejection —
                // fica presa no contexto da página. Sem o try/catch, uma
                // mensagem recebida quebrada trava em silêncio, sem log
                // nenhum no lado do Node.
                try {
                    if (!msg.isNewMsg) return;
                    ...
                    msg.once('change:type', (_msg) => {
                        try {
                            ...
                            console.error(
                                '[wwebjs] change:type handler failed:',
                                err && err.message,
                            );
                        }
                    });
    ...
    async getChatById(chatId) {
        // Chats de contatos com privacidade de LID vêm como "<n>@lid", mas a
        // store interna de Chat indexa esses chats pelo JID de telefone, não
        // pelo LID. Passar o LID direto quebra a busca no IndexedDB.
        if (typeof chatId === 'string' && chatId.endsWith('@lid')) {
            const [resolved] = await this.getContactLidAndPhone([chatId]);
            if (resolved && resolved.pn) chatId = resolved.pn;
        }

        const result = await this.pupPage.evaluate(async (chatId) => {
            try {
                return { ok: true, chat: await window.WWebJS.getChat(chatId) };
            } catch (err) {
                return { ok: false, message: err && err.message, stack: err && err.stack };
            }
        }, chatId);

        if (!result.ok) {
            throw new Error(`getChatById(${chatId}) failed inside page: ${result.message}\n${result.stack}`);
        }
        return result.chat ? ChatFactory.create(this, result.chat) : undefined;
    }
